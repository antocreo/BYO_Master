...
void ofApp::setup(){
    ...
    //some general setup fo the screen and the camera
...
//                    cout << oscSens[j].getAddress() << lastSens << endl;
        ...
        //ofxBullet
        //this part is realted to a future implementation to control 3d physical objects in the 3d space using the Myo.
        //the modality is already implemented but the 3d world is very basic and there are no collisions yet.
        //it is activated with modality four that needs to be activated also in the GUI class
...
void ofApp::draw(){
    ...
    //just be sure that it finds a device otherwise some crash may occur.
    if (myo.getDevices().size()>0) {
        ...
        for ( int j=0; j<NUM_SENS; j++ ) {
            ...
            //let's draw the modalities. here after are the arguments of the functions
            //float *dataPointer, int arraylength, float xpos, float ypos, float zpos, float yMultiplier, float radius, float sides, ofColor fillColor, ofPrimitiveMode mode
...
void ofApp::drawSine(float *dataPointer, int arraylength, float xpos, float ypos, float zpos,
    ...
    //let's create a graph that shows the raw data
    ofPushMatrix();
    ...
    //we setup the vertices and the indices
    ...
    //here is a list of tests I have done to change the look of the graph. some of them works but some other has problems with tassellation.
    // it seems that the values that I get from the Myo in real time, once passed as coordinates, they fail to tassellate the trinagles of the mesh.
    //possibly this is due to the range of the raw values or the speed in which they are updated. this needs more investigation.
    //infact if I generate a mesh with random values, the same code will work nicely.
...
//this modality uses the yMultiplier to separate each ring on the Z axis
...
void ofApp::drawChoir2(float *dataPointer, int arraylength, float xpos, float ypos,float zpos, float yMultiplier,
    ...
    for (int x=0; x<arraylength; x++) {
        ...
        float zp = x * sqrt((pow(myo.getDevices()[0]->getAccel().x,2) +
                             ...
                             pow(myo.getDevices()[0]->getAccel().z,2))/3); //this is not really used in the final output but can be an alternative.
    ...
    //control rotation with Roll/Pitch/Yaw
    ...
    //recentre the shape always and always and always...
...
void ofApp::drawChoir(float *dataPointer, int arraylength, float xpos, float ypos, float zpos,
    ...
    for (int x=0; x<arraylength; x++) {
        ...
        //creating a polygon with the num of side of the buffer size        //this variates the circle on the X,Y axis
        ...
        float zp = zpos + cos(x*angle) * (dataPointer[x]);      //this variates the circle on the Z axis
...
void ofApp::drawBitmapVals(float xpos, float ypos){
    ...
    // ---------------------------------------------------------block taken from the myo example
    for ( int i=0; i<myo.getDevices().size(); i++ ) {
        stringstream s;
   ...
   //------------------------------------------------------end of the code of myo example

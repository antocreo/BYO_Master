...
//    if (myo.getDevices().size()>0) {
        ...
        for ( int j=0; j<NUM_SENS; j++ ) {
            ...
            if (modality == 1) {
                float k = 2;
            drawChoir2(buffer[j], buffersize,  k*rms[j], k*rms[j], k*rms[j] , 10*rms[j], buffersize, ofColor(255/(j+1),100+rms[j], 255-j, 200), OF_PRIMITIVE_POINTS);
            drawChoir2(buffer[j], buffersize,  k*rms[j], k*rms[j], k*rms[j] , 10*rms[j], buffersize, ofColor(255/(j+1),100+rms[j], 255-j, 10*rms[j]), OF_PRIMITIVE_LINE_LOOP);
            }
...
void ofApp::drawChoir2(float *dataPointer, int arraylength, float xpos, float ypos,float zpos,

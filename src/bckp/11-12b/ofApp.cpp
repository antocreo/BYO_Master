...
//                    cout << oscSens[j].getAddress() << endl;
                    if (myIndex2[j]!=0) {
...
void ofApp::draw(){
    ...
    if (myo.getDevices().size()>0) {
        ...
        for ( int a=0; a<ACCEL; a++ ) {
                ...
                ofDrawBitmapString(ofToString(lastAcc), 450, 10+30*a);
            }
        for ( int a=0; a<NUM_SENS; a++ ) {
            
            ofDrawBitmapString(ofToString(lastSens), 450, 100+30*a);
        }
        ...
        if (gui.modality == "one") {
            ...
            for ( int j=0; j<NUM_SENS; j++ ) {
                ...
                ofDrawBitmapString("xpos " + ofToString(xpos), 10, 520);
                ofDrawBitmapString("mouse " + ofToString(ofGetMouseX()), 10, 540);
                ofDrawBitmapString("window " + ofToString(ofGetWidth()), 10, 560);
        ...
        for ( int j=0; j<NUM_SENS; j++ ) {
            ...
            if (gui.modality == "two") {
                ...
                drawChoir2(buffer[j], buffersize,  0, 0, 0, 2*rmsSens[j], 10*rmsSens[j], buffersize, ofColor(255/(j+1),100+rmsSens[j], 255-j, 10*rmsSens[j]), OF_PRIMITIVE_LINE_STRIP);
            }
...
//    }
    ...
    myPulse1.audioOut(outBuffer, 100 * avrgRMSAcc, fabs( myo.getDevices()[0]->getEmgSamples()[0] ), avrgRMSSens);

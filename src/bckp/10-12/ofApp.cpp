    ...
    if (myo.getDevices().size()>0) {
        ...
        for ( int j=0; j<NUM_SENS; j++ ) {
            ...
            if (modality == mode3) {
                ...
                drawSine(buffer[1], buffersize, xpos, ypos+(gap*j), maxWl, maxAmp, ofColor (10*(rms[j]),100,20));
        ...
        for ( int j=0; j<NUM_SENS; j++ ) {
            ...
            if (modality == mode1) {
                float k = 2;
                drawChoir2(buffer[0], buffersize,  0, 0, 0, 2*rms[j], 10*rms[j], buffersize, ofColor(255/(j+1),100+rms[j], 255-j, 200), OF_PRIMITIVE_POINTS);
                drawChoir2(buffer[0], buffersize,  0, 0, 0, 2*rms[j], 10*rms[j], buffersize, ofColor(255/(j+1),100+rms[j], 255-j, 10*rms[j]), OF_PRIMITIVE_LINE_LOOP);
            }
            ...
            if (modality == mode2){
                ...
                drawChoir(buffer[0], buffersize, 0, 0, 0, rms[j] * 10, buffersize, ofColor(255/(j+1),200-rms[j], 100+j, 200), OF_PRIMITIVE_LINE_LOOP);
                drawChoir(buffer[0], buffersize, 0, 0, 0, rms[j] * 10, buffersize , ofColor(255/(j+1),200-rms[j], 100+j, 4*rms[j]), OF_PRIMITIVE_TRIANGLE_STRIP);
